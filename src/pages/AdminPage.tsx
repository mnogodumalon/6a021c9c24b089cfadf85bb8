...
import { TesteingabeViewDialog } from '@/components/dialogs/TesteingabeViewDialog';
...
const ENTITY_TABS = [
  { key: 'testeingabe', label: 'Testeingabe', pascal: 'Testeingabe' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('testeingabe');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'testeingabe': return (data as any).testeingabe as Testeingabe[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'testeingabe': return TESTEINGABE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'testeingabe': return {
        create: (fields: any) => LivingAppsService.createTesteingabeEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTesteingabeEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTesteingabeEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'testeingabe' || dialogState?.entity === 'testeingabe') && (
        <TesteingabeDialog
          open={createEntity === 'testeingabe' || dialogState?.entity === 'testeingabe'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'testeingabe' ? handleUpdate : (fields: any) => handleCreate('testeingabe', fields)}
          defaultValues={dialogState?.entity === 'testeingabe' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Testeingabe']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Testeingabe']}
        />
      )}
      {viewState?.entity === 'testeingabe' && (
        <TesteingabeViewDialog
          open={viewState?.entity === 'testeingabe'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'testeingabe', record: r }); }}
        />
      )}
