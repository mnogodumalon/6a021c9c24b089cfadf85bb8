...
export default function TesteingabePage() {
  const [records, setRecords] = useState<Testeingabe[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Testeingabe | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Testeingabe | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Testeingabe | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getTesteingabe());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Testeingabe['fields']) {
    await LivingAppsService.createTesteingabeEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Testeingabe['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateTesteingabeEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Testeingabe"
      subtitle={`${records.length} Testeingabe im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Testeingabe suchen..."
          value={search}
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Testeingabe. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
