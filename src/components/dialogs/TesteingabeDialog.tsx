...
interface TesteingabeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Testeingabe['fields']) => Promise<void>;
  defaultValues?: Testeingabe['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function TesteingabeDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: TesteingabeDialogProps) {
  const [fields, setFields] = useState<Partial<Testeingabe['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'testeingabe');
      await onSubmit(clean as Testeingabe['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Testeingabe['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Testeingabe bearbeiten' : 'Testeingabe hinzufügen';
