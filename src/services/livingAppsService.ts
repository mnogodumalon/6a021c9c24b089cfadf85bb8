...
import type { Testeingabe, CreateTesteingabe } from '@/types/app';
...
export class LivingAppsService {
  // --- TESTEINGABE ---
  static async getTesteingabe(): Promise<Testeingabe[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TESTEINGABE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Testeingabe[];
    return enrichLookupFields(records, 'testeingabe');
  }
  static async getTesteingabeEntry(id: string): Promise<Testeingabe | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TESTEINGABE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Testeingabe;
    return enrichLookupFields([record], 'testeingabe')[0];
  }
  static async createTesteingabeEntry(fields: CreateTesteingabe) {
    return callApi('POST', `/apps/${APP_IDS.TESTEINGABE}/records`, { fields: cleanFieldsForApi(fields as any, 'testeingabe') });
  }
  static async updateTesteingabeEntry(id: string, fields: Partial<CreateTesteingabe>) {
    return callApi('PATCH', `/apps/${APP_IDS.TESTEINGABE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'testeingabe') });
  }
  static async deleteTesteingabeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TESTEINGABE}/records/${id}`);
  }
